...
add_filter( 'woocommerce_output_related_products_args', 'invictus_related_products_args' );
if(!function_exists('invictus_related_products_args')) {
 function invictus_related_products_args( $args ) {
...
woocommerce_related_products(3); // Display 3 products in rows of 3
}
*/
...
function woo_custom_product_searchform( $form ) {
	...
	$form = '<form role="search" method="get" id="searchform" action="' . esc_url( home_url( '/'  ) ) . '">
		<div>
			...
			<input type="text" class="ins_search_input" value="' . get_search_query() . '" name="s" id="s" placeholder="' . __( 'Search Products', 'invictus' ) . '" />
			<button type="submit" class="ins_submit_btn"  id="searchsubmit" value="'. esc_attr__( 'Search', 'invictus' ) .'" ><i class="fa fa-search" aria-hidden="true"></i></button>
