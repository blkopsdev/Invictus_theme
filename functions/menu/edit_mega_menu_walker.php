...
if (!class_exists('insignia_Edit_Menu_Walker')) {

	class insignia_Edit_Menu_Walker extends Walker_Nav_Menu {
		/**
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$mega_menu_class = INSIGNIA_MEGA_MENU_CLASS;
			$mega_menu_class_e = new $mega_menu_class();
			$mega_menu_class_options = $mega_menu_class_e->insignia_options();
			...
			if ( ! empty( $item->_invalid ) ) {
				...
				$title = sprintf( __( '%s (Invalid)', 'invictus' ), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( __( '%s (Pending)', 'invictus' ), $item->title );
			}
			...
			<li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
				<dl class="menu-item-bar">
					<dt class="menu-item-handle">
						<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo esc_html($submenu_text); ?>><?php _e( 'sub item', 'invictus' ); ?></span></span>
						<span class="item-controls">
							...
							<span class="item-order hide-if-js">
								...
								?>" class="item-move-up"><abbr title="<?php esc_attr_e( 'Move up', 'invictus' ); ?>">&#8593;</abbr></a>
								|
								<a href="<?php
									echo wp_nonce_url(
										add_query_arg(
								...
								?>" class="item-move-down"><abbr title="<?php esc_attr_e( 'Move down', 'invictus' ); ?>">&#8595;</abbr></a>
							</span>
							<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e( 'Edit Menu Item', 'invictus' ); ?>" href="<?php
								echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
							?>"><?php _e( 'Edit Menu Item', 'invictus' ); ?></a>
						</span>
					</dt>
				</dl>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
					...
					<p class="description description-thin">
						<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
							<?php _e( 'Navigation Label', 'invictus' ); ?><br />
							<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
						</label>
					</p>
					<p class="description description-thin">
						<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
							<?php _e( 'Title Attribute', 'invictus' ); ?><br />
					...
					<p class="field-link-target description">
						<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
							...
							<?php _e( 'Open link in a new window/tab', 'invictus' ); ?>
						</label>
					</p>
					<p class="field-css-classes description description-thin">
						<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
							<?php _e( 'CSS Classes (optional)', 'invictus' ); ?><br />
							<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
						</label>
					</p>
					<p class="field-xfn description description-thin">
						<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
							<?php _e( 'Link Relationship (XFN)', 'invictus' ); ?><br />
								...
								<p class="<?php echo esc_attr($block_class); ?>">
									<label for="edit-menu-item-<?php echo esc_attr($option_name); ?>-<?php echo esc_attr($item_id); ?>">
										...
										<?php elseif($params['type']=='upload'): /*Add url check*/?>
											...
											<input id="edit-menu-item-<?php echo esc_attr($option_name); ?>-add-image-<?php echo esc_attr($item_id); ?>-button" class="upload_image_button button" type="button" value="<?php _e('Upload Image','invictus'); ?>" />
											<input id="edit-menu-item-<?php echo esc_attr($option_name); ?>-remove-image-<?php echo esc_attr($item_id); ?>-button" class="remove_image_button button" type="button" value="<?php _e('Remove Image','invictus'); ?>" />
										<?php endif; ?>
									</label>
								</p>
							<?php } ?>
					...
					<p class="field-description description description-wide">
						<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
							<?php _e( 'Description', 'invictus' ); ?><br />
							...
							<span class="description"><?php _e( 'The description will be displayed in the menu if the current theme supports it.', 'invictus' ); ?></span>
						</label>
					</p>

					<p class="field-move hide-if-no-js description description-wide">
						<label>
							<span><?php _e( 'Move', 'invictus' ); ?></span>
							<a href="#" class="menus-move-up"><?php _e( 'Up one', 'invictus' ); ?></a>
							<a href="#" class="menus-move-down"><?php _e( 'Down one', 'invictus' ); ?></a>
							<a href="#" class="menus-move-left"></a>
							<a href="#" class="menus-move-right"></a>
							<a href="#" class="menus-move-top"><?php _e( 'To the top', 'invictus' ); ?></a>
						</label>
					</p>
					...
					<div class="menu-item-actions description-wide submitbox">
						<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
							<p class="link-to-original">
								<?php printf( __( 'Original: %s', 'invictus' ), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
							</p>
						<?php endif; ?>
						...
						); ?>"><?php _e( 'Remove', 'invictus' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
							?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php _e( 'Cancel', 'invictus' ); ?></a>
					</div>
