		...
		<div class="comment-inner <?php echo ($depth == 1 ? 'default-background' : 'bordered-box'); ?>">
			<div class="comment-header clearfix">
				...
				<div class="comment-author vcard allcomments-img-box">
					...
					<?php printf(wp_kses(__('<div class="fn title-h6">%s</div>', 'invictus'), array('div' => array('class' => array()))), get_comment_author_link()); ?>
					<div class="comment-meta commentmetadata date-color allcomments-text-time-day"><a href="<?php echo esc_url(get_comment_link($comment->comment_ID, $args)); ?>">
						<?php
							...
							printf(esc_html__('%1$s at %2$s', 'invictus'), get_comment_date(),  get_comment_time()); ?></a><?php edit_comment_link(esc_html__('(Edit)', 'invictus'), '&nbsp;&nbsp;', '');
						?>
					</div> </div>
				
			</div>
			<?php if('0' == $comment->comment_approved) : ?>
			<div class="comment-awaiting-moderation"><?php esc_html_e('Your comment is awaiting moderation.', 'invictus') ?></div>
			<?php endif; ?>
