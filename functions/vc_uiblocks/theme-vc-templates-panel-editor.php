...
class Invictus_Vc_Templates_Panel_Editor {
	/**
	 * @since 4.4
	 * @var bool
	 */
	...
	public function addTemplatesTab( $data ) {
		$newCategory = array(
			...
			'category_name'   => esc_html__( 'Invictus Templates', 'invictus' ),
			'category_weight' => 1,
	...
	protected function get_template_categories() {
		...
		$categories = array(
			'all'					=> esc_html__( 'All', 'invictus' ),
			'about'					=> esc_html__( 'About', 'invictus' ),
			'blog'					=> esc_html__( 'Blog', 'invictus'),
			'hero-sections'      	                => esc_html__( 'Hero Sections', 'invictus' ),
			'portfolio'				=> esc_html__( 'Portfolio', 'invictus' ),
			'counters'				=> esc_html__( 'Counters', 'invictus' ),			
			'icon-box'				=> esc_html__( 'Icon Boxes', 'invictus' ),			
			'clients'				=> esc_html__( 'Clients', 'invictus' ),
			'pricing'				=> esc_html__( 'Pricing', 'invictus' ),
			'service'				=> esc_html__( 'Services', 'invictus' ),			
			'team'					=> esc_html__( 'Teams', 'invictus' ),
			'testimonial'			        => esc_html__( 'Testimonials', 'invictus' ),
			'image-gallery'			        => esc_html__( 'Image Gallery', 'invictus' ),			
			'contact'				=> esc_html__( 'Contact', 'invictus' ),	
			'call-to-action'			=> esc_html__( 'Call to Action', 'invictus' ),
			'extra'					=> esc_html__( 'Extra', 'invictus' ),
	...
	function renderTemplateWindow( $template_name, $template_data ) {
		...
		if ( 'invictus_templates' === $template_data['type'] ) {
			return $this->renderTemplateWindowInvictusTemplates( $template_name, $template_data );
		}
	 ...
	 * @return string
	 */
	public function renderTemplateWindowInvictusTemplates( $template_name, $template_data ) {
		ob_start();
		...
		$preview_template_title = esc_attr( 'Preview template', 'invictus' );
		$add_template_title = esc_attr( 'Add template', 'invictus' );
...
HTML;
	...
	public function renderBackendTemplate() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( 'Error: Vc_Invictus_Templates::renderBackendTemplate:1' );
		}
	...
	public function renderTemplateListItem( $template ) {
		$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'invictus' ) );
